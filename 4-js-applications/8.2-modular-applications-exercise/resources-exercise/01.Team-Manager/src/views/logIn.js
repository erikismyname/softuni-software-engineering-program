import { html } from '../../node_modules/lit-html/lit-html.js';
import { logInUser } from '../services/userService.js';

const logInPageTemplate = (onUserLogIn, errorMsg) => html`
    <section id="login">
        <article class="narrow">
            <header class="pad-med">
                <h1>Login</h1>
            </header>
            <form @submit=${onUserLogIn} id="login-form" class="main-form pad-large">
                ${errorMsg ? html`<div class="error">${errorMsg}</div>` : ''}
                <label>E-mail: <input type="text" name="email"></label>
                <label>Password: <input type="password" name="password"></label>
                <input class="action cta" type="submit" value="Sign In">
            </form>
            <footer class="pad-small">Don't have an account? <a href="/register" class="invert">Sign up here</a>
            </footer>
        </article>
    </section>
`;

export async function displayLogInPage(ctx) {

    ctx.render(logInPageTemplate(onUserLogIn));

    async function onUserLogIn(ev) {

        ev.preventDefault();

        const formData = new FormData(ev.target);

        const email = formData.get('email').trim();

        const password = formData.get('password').trim();

        try {

            await logInUser({ email, password });

            ev.target.reset();

            ctx.setUserNav();

            ctx.page.redirect('/my-teams');

        } catch (err) {

            ctx.render(logInPageTemplate(onUserLogIn, err));

        }

    }

}